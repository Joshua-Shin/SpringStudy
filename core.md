...
#### 빈 생명주기 콜백
- 콜백(callback) : 돌아오면 전화해 달라는 부탁
- 스프링 빈의 이벤트 라이프사이클
  - 스프링컨테이너생성 -> 스프링빈생성 -> 의존관계주입 -> 초기화콜백 -> 사용 -> 소멸전콜백 -> 스프링 종료
- 객체의 생성과 (외부와의 연결 같은)초기화 과정을 분리. 단일 책임 원칙.
- 초기화 콜백: 빈이 생성되고, 빈의 의존관계 주입이 완료된 후 호출. 본격적으로 사용하기 직전, 추가적인 행동을 할 수 있도록 하는거. 예를 들면 db커넥션풀이나 네트워크소켓과의 연결 같은거.
- 소멸전 콜백: 소멸전 콜백: 빈이 소멸되기 직전에 호출.
- 솔직히 왜 초기화 콜백과 소멸전 콜백이 왜 필요한지 어느상황에 필요한지 아직 와닿지가 않네.
- 빈 생명주기 콜백 방법
  - 1_ @Bean(initMethod = "init", destroyMethod = "close")
    - 빈 수동 등록 하는 과정에서 초기화메소드와 종료메소드를 지정할 수 있음. init와 close는 해당 빈 객체의 메소드명임.
    - 수정할 수 없는 외부라이브러리를 사용할 경우 내가 설정 클래스를 수동으로 제어할 수 있으니 이 방법이 좋음.
  - 2_ @PostConstruct, @PreDestroy
    - 빈 객체의 메소드레벨에 해당 애노테이션 선언. 가장 간편.
    - 다만, 수정할 수 없는 외부라이브러리를 끌어다 쓸 경우 할 방법이 없음.
- 결론
...
- 지금껏 따로 빈 스코프를 명시하지 않았는데, 명시하지 않을 경우 디폴트로 싱글톤 스코프임. 스프링 컨테이너와 거의 생명주기를 같이함.
- 종류 : 싱글톤, 프로토타입, 웹스코프(request, session, application)
- 빈 등록할 클래스 레벨에 @Scope("prototype") 명시
- 사실 거의 쓸일 없음. 거의 싱글톤만 씀.
- 프로토타입 스코프
  - 빈 조회를 요청할 때마다 새로운 객체를 생헝해서 의존관계 주입 및 초기화 이후 반환해줌. 반환한 이후로는 스프링 컨테이너가 관리 안함.
  - 따라서, @PreDestroy로 선언한 종료 메소드는 스프링 컨테이너가 종료될때 호출 안됨.
- 싱글톤 빈이 프로토 타입 빈을 주입 받았을때.
  - 프로토 타입을 주입 받을때 한번 생성될 뿐, 당연하게도 싱글톤빈안에서 사용할때마다 새로 생성되지 않게됨.
  - 그런데 이건 의도한 바가 아님. 이럴거면 그냥 싱글톤으로 둘다 선언하지 뭣하고 프로토 타입으로 썼겠어.
  - 때문에 싱글톤 안에서 해당 프로토타입 빈 객체를 사용할때마다 새로 생성하고 싶다면, 싱글톤안에서 스프링 컨테이너를 주입받아서 ac.getBean("PrototypeBean.class").
  - 하면 가능은 하지만 스프링 컨테이너를 주입한다는건 너무 무식한 방법이고, 스프링 컨테이너에서 직접 찾는게 아니라, 딱 빈을 찾아주는 기능만을 위한 애가 있음
- ObjectProvider<PrototypeBean> prototypeBeanProvider; 
  - 싱글톤 안에서 스프링 컨테이너를 주입받는게 아니라, 얘를 주입받아서, 프로토 타입의 객체를 사용하고 싶을떄마다 prototypeBeanProvider.getObject() 해서 꺼내오면 매번 새로운 객체가 생성되어 꺼내짐. 이걸 Dependency Lookup. DL 이라 함.
  - 얘는 스프링에서 제공하는애고 자바 표준으로 Provider 라는 애가 있는데, 그냥 얘 써.
- 웹 스코프 중 request 스코프
  - http 요청이 올때 생성되고, 응답으로 나갈때까지 소멸됨. 요청때마다 객체가 새로운 객체가 생성.
- request 스코프로 설정된 빈 객체를 myLogger라 할때.
  - myLogger를 Controller 클래스에 필드로 두면, 오류 발생
  - 서버가 띄워질때 아직 http 요청이 안온 상태이기에, myLogger는 생성이 안되었기에, 생성자주입시 오류가 발생하는거.
  - 따라서 얘를 그냥 서비스 계층으로 올려도 되긴하지만, 서비스 계층은 되도록 비즈니스 로직만 두고 웹과 관련된건 Controller에 두는것이 유지보수에 좋아.
  - 그래서 Controller 계층에서 해결을 봐야함.
  - 1_ 해결방법
    - 필드로 주입받게 하지 말고, 필드로는 ObjectProvider<MyLogger> prototypeBeanProvider; 를 두는거야.
    - @GetMapping 선언한 메소드 안에서 필요한 순간 prototypeBeanProvider에서 myLogger를 꺼내쓰게.
  - 2_ 해결방법
    - @Scope(value = "request", proxyMode = ScopedProxyMode.TARGET_CLASS)
    - MyLogger를 프록시모드로 설정. 이리하면 일단 주입 단게에서는 가짜 프록시 개체가 들어가고, 실제 요청이 필요할때 가짜놈이 그때서야 진짜 객체의 메소드를 호출해줌.
  - 두 방법 다 꼭 필요한 시점까지 지연 처리 한다는게 핵심.  
